...
class BreakevenPCABetaFactor(InflationDataManager):
    
    def __init__(self) -> None:
        
        super().__init__()
        self.breakeven_path = os.path.join(self.data_path, "BreakevenPCABetaFactor")
        if os.path.exists(self.breakeven_path) == False: os.makedirs(self.breakeven_path)
        
        self.comps = 3
        
    def get_breakeven_pca(self, verbose: bool = False) -> pd.DataFrame: 
        
        file_path = os.path.join(self.breakeven_path, "PCAFittedValues.parquet")
        try:
            ...
            df_wider = (self.get_breakeven().drop(
                columns = ["Description"]).
    ...
    def get_pca_regression(self, verbose: bool = False, window: int = 30) -> pd.DataFrame: 
        ...
        file_path = os.path.join(self.breakeven_path, "PCARegressedValues.parquet")
        try:
            ...
            df_out = (self.get_breakeven_pca().diff().dropna().reset_index().melt(
    ...
    def get_deciles(self, n_tiles: int = 5, verbose: bool = False) -> pd.DataFrame: 
        ...
        file_path = os.path.join(self.breakeven_path, "SecurityDeciles.parquet")
        try:
    ...
    def generate_daily_factor(self, verbose: bool = False) -> pd.DataFrame: 
        ...
        file_path = os.path.join(self.breakeven_path, "DailyFactor.parquet")
        try:
    ...
    def generate_monthly_factor(self, verbose: bool = False) -> pd.DataFrame: 
        ...
        file_path = os.path.join(self.breakeven_path, "MonthlyFactor.parquet")
        try:
...
def main() -> None:
    ...
    df = BreakevenPCABetaFactor().get_breakeven_pca(verbose = True)
    df = BreakevenPCABetaFactor().get_pca_regression(verbose = True)
    df = BreakevenPCABetaFactor().get_deciles(verbose = True)
    df = BreakevenPCABetaFactor().generate_daily_factor(verbose = True)
    df = BreakevenPCABetaFactor().generate_monthly_factor(verbose = True)
